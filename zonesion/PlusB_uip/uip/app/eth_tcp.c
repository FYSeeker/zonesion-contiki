...
char TcpSocket_OutBuf[TCP_SOCKETOUT_BUF_SIZE] = {0};
char TcpSocket_InBuf[TCP_SOCKETIN_BUF_SIZE] = {0};
...
void TpcKeyHandle(uint8_t value)
{
    switch(value)
    {
        ...
        case 0x08:
            if(tcp_socket_connect(&TcpSocket,&tcpCB.RemoteIP,tcpCB.RemotePort))
...
PROCESS_THREAD(eth_tcp, ev, data)
{
    ...
    tcp_socket_register(&TcpSocket,
                        ...
                        TCP_SOCKETOUT_BUF_SIZE,
                        (uint8_t*)TcpSocket_InBuf,
                        TCP_SOCKETIN_BUF_SIZE,
