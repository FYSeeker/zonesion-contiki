...
void lcdInitPage(char x,short tiem)
{
    ...
    {
        LCDShowFont16(5,102,"初始化完成，加载界面...",300,LCD_COLOR_WHITE,LCD_COLOR_BLACK);
        lcdEvent_InitFlag=4;
    }
}
...
void lcdShowPage1(unsigned char refresh)
{
    switch(hwType_Get())
    {
		case 0:
...
void lcdShowPage2(unsigned char refresh)
{
    uint16_t x=0,y=0;
    char pbuf[50]= {0};
    ...
    {
        LCDClear(LCD_COLOR_WHITE);
        lcdShowBarX1(y,"测试说明",LCD_COLOR_WHITE,LCD_COLOR_RED,refresh,1);

        sprintf(pbuf,"1-LED/RGB");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_RED,LCD_COLOR_WHITE);
        sprintf(pbuf,"  LED周期闪烁,RGB周期切换三种颜色。");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);

        sprintf(pbuf,"2-按键");
        LCDShowFont16(x,y+=30,pbuf,strlen(pbuf)*8,LCD_COLOR_RED,LCD_COLOR_WHITE);
        sprintf(pbuf,"  若程序检测到按键按下会显示正常；");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);
        sprintf(pbuf,"  K1显示正常后，按下切换到测试说明页；");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);
        sprintf(pbuf,"  K2显示正常后，按下重新进行测试；");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);
        sprintf(pbuf,"  K3显示正常后，按下开/关继电器一；");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);
        sprintf(pbuf,"  K4显示正常后，按下开/关继电器二。");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);

        sprintf(pbuf,"3-以太网口");
        LCDShowFont16(x,y+=30,pbuf,strlen(pbuf)*8,LCD_COLOR_RED,LCD_COLOR_WHITE);
        sprintf(pbuf,"  网口通过网线连接到路由器LAN口。");
        LCDShowFont16(x,y+=20,pbuf,strlen(pbuf)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);
    }
}

void lcdShowUpdate(unsigned char refresh)
{
    if(lcdPageIndex)
    {
        lcdShowPage2(refresh);
    }
    else
    {
        lcdShowPage1(refresh);
    }
}
...
void lcdKeyHandle(unsigned char keyValue)
{
    ...
    if(keyValue==0x01)
    {
        ...
        {
            lcdPageIndex++;
            if(lcdPageIndex>1) lcdPageIndex=0;
