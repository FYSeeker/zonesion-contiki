...
//flash
uint16_t flash_startAddr = 1;                                   //起始位置
uint8_t flash_write_buff[12] = {"Hello World!"};
uint8_t flash_read_buff[12];
...
PROCESS_THREAD(test_plus, ev, data)
{
    ...
    process_start(&rfUartProcess,NULL);
...
PROCESS_THREAD(test_init, ev, data)
{
    ...
    process_start(&KeyProcess,NULL);
    etimer_set(&etimer_testInit, 200);
