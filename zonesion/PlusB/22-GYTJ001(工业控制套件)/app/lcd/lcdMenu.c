...
{
    LCD_DrawLine(8,y+1,312,y+1,LCD_COLOR_GRAY_4);
    LCD_DrawLine(7,y+1,7,y+18,LCD_COLOR_GRAY_4);
...
void lcdShowTable(unsigned char refresh)
{
    ...
    for(i=0;i<5;i++)
    {
        y+=20;
        mode = (i==4)?0:1;
        lcdShowMenuBar_3(y,TableStruct.name[i],TableStruct.explain[i],TableStruct.pdata[i],LCD_COLOR_BLACK,LCD_COLOR_GRAY_1,refresh,mode);
    }
}
