            ...
            for (int j=0; j<24; j++)
            {
                for (int k=0; k<3; k++)
                {
                    ...
                    for (int m=0; m<8; m++)
                    {
                        if (dot & 1<<(7-m))
                        {
                            LCD_DrawPixel(x+k*8+m, y+j, color);
                        }
                        else  if (bc > 0)
                        {
                            LCD_DrawPixel(x+k*8+m, y+j, bc);
                        }
                    }
                }
            }
...
* 注释：
*********************************************************************************************/
void LCD_File(int x1,int y1,int x2,int y2,uint16_t color)
{
...
void LCD_Color_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u8 *color)
{
    ...
    for(y=0; y<height; y+=1)
    {
        for(x=0; x<width; x+=1)
        {
            color_temp = color[2*(y*width+x)]<<8;
            color_temp |= color[2*(y*width+x)+1];
            LCD_DrawPixel(sx+x,sy+y,color_temp);//画点
        }
    }
}
...
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode,u16 color,u16 backColor)
{
    ...
    for(t=0; t<csize; t++)
    {
        ...
        for(t1=0; t1<8; t1++)
        {
            ...
            else if(mode==0)LCD_DrawPixel(x,y,backColor);
            temp<<=1;
            y++;
    ...
    for(b=0; b<1; b++)
    {
        ...
        for(i=0; i<16; i++)
        {
            ...
            for(j=0; j<8; j++)
            {
                ...
                if ( (tmp_char << j) & 0x80)
                {
                    LCD_DrawPixel(x+j+8,y+i,Color);
                }
                else
                {
                    LCD_DrawPixel(x+j+8,y+i,BColor);
                }
            }
        }
    }
...
* 名称：LcdShowIcon
* 功能：lcd显示图标
* 参数：x1,y1:起始坐标值，dat：图标数据（80X60），value：图标下面的文字
* 返回：
* 修改：
* 注释：
*********************************************************************************************/
...
{
    LCD_Color_Fill(x,y,x+80,y+60,dat);
    LCD_File(x+1,y+62,x+79,y+78,LCD_COLOR_WHITE);
    LCDShowFont16(x+(40-(strlen((char*)value)*4)),y+62,(char*)value,strlen((char*)value)*8,LCD_COLOR_BLACK,LCD_COLOR_WHITE);
}