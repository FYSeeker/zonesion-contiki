...
void ui_drawSignal(uint16_t x,uint16_t y,uint8_t size,uint8_t status)
{
    ...
    if(status)
    {
        LCD_FillColor(x,y,x+size-1,y+size-1,LCD_COLOR_BLACK);
        LCD_FillColor(x,y+(size/3*2),x+(size/5),y+size-1,color);
        LCD_FillColor(x+(size/5*2),y+(size/3),x+(size/5*3),y+size-1,color);
        LCD_FillColor(x+(size/5*4),y,x+size,y+size-1,color);
    }
    else
    {
...
void ui_DrawBatteryIcon(uint16_t x,uint16_t y,uint8_t size,uint8_t quantity)
{
    ...
    LCD_FillColor(x+2,y+2,x+quantity+2,y+size-3,color);
    LCD_FillColor(x+quantity+2,y+2,x+size*2-5,y+size-3,LCD_COLOR_BLACK);
}
...
void ui_showStatusBar(uint8_t redraw)
{
    ...
    if(redraw)
    {
        LCD_FillColor(0,0,320,20,backColor);
    }
