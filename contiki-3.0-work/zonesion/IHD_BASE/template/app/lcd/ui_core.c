...
void ui_drawIcon(uint16_t x,uint16_t y,UI_Icon_t* icon)
{
    ...
    if(icon->style)
    {
        ...
        LCD_FillColor(x,y,x+(icon->wide/4),y+(icon->high/4),color);
        LCD_FillColor(x+(icon->wide/4*3),y,x+icon->wide-1,y+(icon->high/4),color);
        LCD_FillColor(x,y+(icon->high/4*3),x+(icon->wide/4),y+icon->high-1,color);
        LCD_FillColor(x+(icon->wide/4*3),y+(icon->high/4*3),x+icon->wide-1,y+icon->high-1,color);
    }
    else
    {
        LCD_FillColor(x,y,x+icon->wide-1,y+icon->high-1,LCD_COLOR_WHITE);
    }
    
    if(icon->bitmap != NULL)
    {
        LCDSetWindow(x+space,x+icon->wide-space-1,y+space,y+icon->high-space-1);
        LCDSetCursor(x+space, y+space);
        ...
        LCD_FillColor(x+space,y+space,x+icon->wide-space-1,y+icon->high-space-1,LCD_COLOR_CYAN);
}
...
uint8_t ui_drawWindow(uint16_t x,uint16_t y,uint8_t redraw,Window_t* window)
{
    ...
    if(redraw)
    {
        LCD_FillColor(x+2,y+2,x+window->wide-2,y+barHigh+2,window->barColor);
        LCD_FillColor(x+2,y+barHigh+2,x+window->wide-2,y+window->high-2,window->boxColor);
