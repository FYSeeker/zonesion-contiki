...
void xLabPageShow(uint8_t refresh)
{
    ...
    uint16_t titleColor = LCD_COLOR_RED;
    char pbuf1[32]={0};
    char pbuf2[32]={0};
    ...
    {
        case 1:
            titleColor = LCD_COLOR_RED;
            break;
        case 2:
            titleColor = LCD_COLOR_BLUE;
            break;
        case 3:
            titleColor = LCD_COLOR_DARK_GREEN;
            break;
        case 4:
            titleColor = LCD_COLOR_PURPLE;
            break;
        case 5:
            titleColor = LCD_COLOR_ORANGE;
            break;
    }
    ...
    if(refresh&0x80)
    {
        ...
        lcdShowMenuBar_1(0,"xLab信息",LCD_COLOR_WHITE,titleColor,refresh,1);
    }
    lcdShowMenuBar_1(y+=30,"传感器数据",LCD_COLOR_WHITE,LCD_COLOR_BLACK,refresh,1);
    
    sprintf(pbuf1,"%5.1f",A0);
    sprintf(pbuf2,"%5.1f",A1);
    lcdShowBarX4(y+=20,"温度",pbuf1,"湿度",pbuf2,penColor,backColor,refresh,1);
    
    sprintf(pbuf1,"%5.1f",A2);
    sprintf(pbuf2,"%6.1f",A3);
    lcdShowBarX4(y+=20,"光照强度",pbuf1,"空气质量",pbuf2,penColor,backColor,refresh,1);
    
    sprintf(pbuf1,"%5.1f",A4);
    sprintf(pbuf2,"%5.1f",A6);
    lcdShowBarX4(y+=20,"大气压力",pbuf1,"红外测距",pbuf2,penColor,backColor,refresh,1);
