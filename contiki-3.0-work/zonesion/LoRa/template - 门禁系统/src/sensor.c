...
static char A0[8] = {0};                                        // A0存储燃气浓度值
static uint16_t V0 = 30;                                        // V0设置为上报时间间隔，默认为30s
static uint8_t V1 = 2;                                          // 默认允许保存2张
extern char ICCard[9];
/*********************************************************************************************
* 名称：updateV0()
* 功能：更新V0的值
* 参数：*val -- 待更新的变量
...
PROCESS_THREAD(sensor, ev, data)
{
  ...
  while (1) {
    ...
    if (etimer_expired(&et_check)) {
      sensorCheck();
      etimer_set(&et_check, get_readTime());
    }
