...
static uint8_t D1 = 1;                                          // 继电器初始状态为全关
static uint16_t A0 = 0;                                         // CO2
static uint8_t A1 = 0;                                          // VOC等级/甲醛
static float A2 = 0;                                            // 湿度
static float A3 = 0;                                            // 温度
static uint16_t A4 = 0;                                         // PM2.5
...
{ 
  ...
  if ((D0 & 0x10) == 0x10){                                     // 若湿度上报允许，则pData的数据包中添加湿度数据
    updateA4();
    sprintf(p, "%d", A4); 
    ZXBeeAdd("A4", p);
  }
...
PROCESS_THREAD(sensor, ev, data)
{
  ...
  while (1) {
    ...
    if (etimer_expired(&et_check)) {
      sensorCheck();
      etimer_set(&et_check, CLOCK_SECOND*3);
    }
