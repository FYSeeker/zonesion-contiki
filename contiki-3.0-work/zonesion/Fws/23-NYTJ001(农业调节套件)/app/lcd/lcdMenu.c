                       ...
                       unsigned short backColor_1,unsigned short backColor_2,unsigned char refresh)
{
    if(refresh&0x80)
    {
        LCD_File(0,y,319,y+19,backColor_1);
        if(hwType_Get()==0)
        {
    ...
    lcdShowBarX3(y+40,&sensorDataBuf.pdata[0][0],&sensorDataBuf.pdata[1][0],&sensorDataBuf.pdata[2][0],wordColor,backColor_2,refresh,1);
}