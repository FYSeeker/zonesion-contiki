...
void lcd_Update(uint8_t refresh)
{
    ...
    {
        sprintf(pbuf,"K3:关闭服务");
        LCDShowFont16(10+(50-strlen(pbuf)*4),y+2,pbuf,strlen(pbuf)*8,PenColor,BackColor);
    }
    else
    {
        sprintf(pbuf,"K3:打开服务");
        LCDShowFont16(10+(50-strlen(pbuf)*4),y+2,pbuf,strlen(pbuf)*8,PenColor,BackColor);
    }
    
    sprintf(pbuf,"K4:连接服务");
    LCDShowFont16(10+(150-strlen(pbuf)*4),y+2,pbuf,strlen(pbuf)*8,PenColor,BackColor);
    
    if(TcpStatusGet() == STA_CONNECT)
        sprintf(pbuf,"状态:已连接");
    else
        sprintf(pbuf,"状态:已断开");
    LCDShowFont16(10+(250-strlen(pbuf)*4),y+2,pbuf,strlen(pbuf)*8,PenColor,BackColor);
    ...
    if(TcpDataBuf[0] != NULL)
        LCDShowFont16(8,y+2,TcpDataBuf,301,PenColor,BackColor);
    else
